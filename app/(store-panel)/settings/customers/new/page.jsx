    ...
    return (
      <DashboardLayout>
        <div style={s.deniedWrap}>
          ...
          <p style={s.deniedText}>You do not have permission to add customers.</p>
        </div>
      </DashboardLayout>
    )
  }
      ...
      <div style={{ ...s.wrapper, maxWidth: isMobile ? '100%' : 940 }}>
        <div style={s.pageHeader}>
          <div style={{ ...s.headerLeft, width: isMobile ? '100%' : 'auto' }}>
            <button type="button" style={s.backBtn} onClick={() => router.push('/settings/customers')}>
              <ArrowLeft size={16} />
            </button>
            <div>
              <h1 style={{ ...s.pageTitle, fontSize: isMobile ? 20 : 30 }}>Add Customer</h1>
              <p style={{ ...s.pageSubtitle, fontSize: isMobile ? 12 : 13.5 }}>Create a customer entry with contact details</p>
            </div>
          </div>
        ...
        <div style={{ ...s.card, borderRadius: isMobile ? 14 : 20, padding: isMobile ? 14 : 24 }}>
          ...
          <div style={s.footer}>
            <button type="button" style={{ ...s.cancelBtn, width: isMobile ? '100%' : 'auto' }} onClick={() => router.push('/settings/customers')}>Cancel</button>
            <button type="button" style={{ ...(saving ? s.saveBtnDisabled : s.saveBtn), width: isMobile ? '100%' : 'auto' }} onClick={handleSave} disabled={saving}>
              <Save size={15} /> {saving ? 'Saving...' : 'Save Customer'}
            </button>
          </div>
