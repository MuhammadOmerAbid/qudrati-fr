...
export const REPORT_TABLE_BORDER = '#000000'
  ...
  const groupedGateOutwardHtml = (() => {
    ...
    return groups.map((group) => {
      ...
      const metaCells = goColumns.map((col) => {
        ...
        return `<td><span>${escapeHtml(col.label)}</span><strong>${escapeHtml(formatReportValue(raw))}</strong></td>`
      })
      const metaRows = []
      for (let index = 0; index < metaCells.length; index += 5) {
        const cells = metaCells.slice(index, index + 5)
        while (cells.length < 5) cells.push('<td><span>&nbsp;</span><strong>&nbsp;</strong></td>')
        metaRows.push(`<tr>${cells.join('')}</tr>`)
      }
      const itemHead = itemColumns.map((col) => `<th>${escapeHtml(col.label)}</th>`).join('')
      const itemColGroup = itemColumns.map((col) => (
        `<col${itemColumnWidth(col) ? ` style="width:${escapeHtml(itemColumnWidth(col))}"` : ''} />`
      )).join('')
      ...
      return `
        <section class="go-card">
          <table class="go-meta-table"><tbody>${metaRows.join('')}</tbody></table>
          <table class="go-items">
            <colgroup>${itemColGroup}</colgroup>
  ...
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .table-shell { border: 1px solid ${REPORT_TABLE_BORDER}; overflow: hidden; background: #fff; }
      table { width: 100%; border-collapse: collapse; table-layout: ${isWideReport ? 'fixed' : 'auto'}; }
      th { background: #f8fafc; color: #0f172a; text-align: left; font-size: 10.5px; border: 1px solid ${REPORT_TABLE_BORDER}; padding: 9px 8px; font-weight: 800; text-transform: none; letter-spacing: 0; overflow-wrap: normal; word-break: normal; }
      td { border: 1px solid ${REPORT_TABLE_BORDER}; padding: 8px 8px; vertical-align: top; color: #273529; font-weight: 400; line-height: 1.35; background: #fff; overflow-wrap: break-word; word-break: normal; }
      .entry-cell { vertical-align: middle; font-weight: 400; color: #273529; background: #fff; }
      .group-start td { border-top: 1px solid ${REPORT_TABLE_BORDER}; }
      .group-start .entry-cell { border-left: 1px solid ${REPORT_TABLE_BORDER}; background: #fff; }
      .group-cont td { border-top: 1px solid ${REPORT_TABLE_BORDER}; }
      tr:hover td { background: #f8fafc; }
      .empty { text-align: center; color: #64748b; padding: 22px; }
      .go-card { border: 1px solid ${REPORT_TABLE_BORDER}; margin-bottom: 14px; background: #fff; page-break-inside: avoid; break-inside: avoid; }
      .go-meta-table { width: 100%; table-layout: fixed; border-collapse: collapse; background: #eef6fc; }
      .go-meta-table td { width: 20%; min-height: 48px; padding: 8px 10px; border: 1px solid ${REPORT_TABLE_BORDER}; border-top: none; vertical-align: top; background: #eef6fc; }
      .go-meta-table tr:first-child td { border-top: 1px solid ${REPORT_TABLE_BORDER}; }
      .go-meta-table span { display: block; color: #5b6b79; font-size: 9px; font-weight: 800; text-transform: uppercase; letter-spacing: .35px; }
      .go-meta-table strong { display: block; margin-top: 3px; color: #0f2f4a; font-size: 12px; font-weight: 400; line-height: 1.25; overflow-wrap: break-word; word-break: normal; }
      .go-items { table-layout: fixed; }
      .go-items th { background: #f8fafc; color: #0f172a; border: 1px solid ${REPORT_TABLE_BORDER}; font-size: 10px; letter-spacing: .2px; }
      .go-items td { border-color: ${REPORT_TABLE_BORDER}; font-size: 11px; line-height: 1.25; overflow-wrap: break-word; word-break: normal; }
      ...
      @media print {
        ...
        th { background: #f8fafc !important; color: #0f172a !important; border-color: ${REPORT_TABLE_BORDER} !important; font-size: ${isWideReport ? '7.2px' : '9.2px'}; padding: ${isWideReport ? '4px 4px' : '7px 6px'}; letter-spacing: 0; line-height: 1.15; }
        td { background: #fff !important; border-color: ${REPORT_TABLE_BORDER} !important; font-size: ${isWideReport ? '7.4px' : '9.5px'}; padding: ${isWideReport ? '4px 4px' : '6px 6px'}; line-height: 1.16; overflow-wrap: break-word; word-break: normal; }
        .entry-cell { background: #fff !important; border-left-color: ${REPORT_TABLE_BORDER} !important; }
        .group-start td { border-top-color: ${REPORT_TABLE_BORDER} !important; }
        .footer { margin-top: 10px; }
        .go-card { margin-bottom: 8px; }
        .go-meta-table { table-layout: fixed !important; background: #eef6fc !important; }
        .go-meta-table td { width: 20% !important; min-height: 34px; padding: 5px 6px; background: #eef6fc !important; border-color: ${REPORT_TABLE_BORDER} !important; }
        .go-meta-table span { font-size: 7px; }
        .go-meta-table strong { font-size: 8.5px; }
